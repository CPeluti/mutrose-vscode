...
export class GoalModelProvider implements vscode.TreeDataProvider<
  void | GoalModel | Mission | Node | NodeAttr | Refinement | NodeRefinement
> {
  ...
  getChildren(
    element?: GoalModel | Mission | Node | undefined,
  ): Thenable<
    | GoalModel[]
    | Mission[]
    | Node[]
    | Refinement[]
    | (NodeAttr | NodeRefinement)[]
  > {
  ...
  private getGoalModels(gmFolderPath: string): GoalModel[] {
    ...
    if (this.pathExists(gmFolderPath) && workspaceRoot) {
      ...
      const gms: { gm: string; filePath: string }[] = gmList.map((gm) => {
        return {
          gm: fs.readFileSync(path.join(gmFolderPath, gm)).toString(),
          filePath: path.join(gmFolderPath, gm),
        };
      });
      const parsedGMs: { gm: gmTypes.GoalModel; filePath }[] = gms.map(
        ({ gm, filePath }) => {
          const res = JSON.parse(gm);
          return { gm: res as gmTypes.GoalModel, filePath };
        },
      );
      const goalModels = parsedGMs.map(
        (gm: { gm: gmTypes.GoalModel; filePath: string }) => {
...
export class NodeRefinement extends vscode.TreeItem {
  ...
  constructor(
    ...
    public refinementsToInstantiate: Array<{
      tag: string;
      customId: string;
      linkId: string;
    }>,
  ...
  addRefinement(targetId, tag, newId): void {
    this.refinements.push(
      new Refinement(
        { tag, linkId: newId, customId: targetId },
        vscode.TreeItemCollapsibleState.None,
        this,
      ),
    );
  ...
  parseRefinements(): gmTypes.Link[] {
    return this.refinements.map((r) =>
      r.parseLink(this.parent.customId, this.type),
    );
  }
  removeRefinement(refinement: Refinement) {
    this.refinements = this.refinements.filter((e) => e != refinement);
  }
  changeRefinementType(
    newRefinementType: "istar.AndRefinementLink" | "istar.OrRefinementLink",
  ) {
...
export class Node extends vscode.TreeItem {
  ...
  parseNode(): gmTypes.Node {
    const customProperties: Record<string, string> = this.attributes
  ...
  getRefinements() {
    const links = this.parent.parent.links.filter(
      (link) => link.target === this.customId,
    );
    if (links.length) {
      ...
      this.refinements = new NodeRefinement(
        type,
        aux,
        vscode.TreeItemCollapsibleState.Collapsed,
        this,
  ...
  setRuntimeAnnotation(annotation: string) {
    ...
    this.description =
      this.tag.trim() +
      (this.runtimeAnnotation != "" ? ` [${this.runtimeAnnotation}]` : "");
    return;
  }
...
export class Mission extends vscode.TreeItem {
  ...
  constructor(
    ...
    const nodes = nodesToInstatiate.map((node) => {
      ...
      const runtimeAnnotationCheck = tag?.match(
        /(?<=\[)[a-zA-Z\d,|#|;?]*(?=\])/g,
      );
      const newTag = tag.replace(/\[[a-zA-Z\d,|#|;?]*\]/g, "");
      const nodeInst = new Node(
        name,
        [],
        vscode.TreeItemCollapsibleState.Collapsed,
        newTag,
        name.startsWith("AT") ? "Task" : "Goal",
        this,
        node.id,
        { x: node.x, y: node.y },
        runtimeAnnotationCheck ? runtimeAnnotationCheck[0] : "",
      );
  ...
  addNewNode(type: "Task" | "Goal", title: string) {
    const name =
      type === "Task" ? `AT${this.lastTaskNumber}` : `G${this.lastGoalNumber}`;
    const nodeInst = new Node(
      name,
      [],
      vscode.TreeItemCollapsibleState.Collapsed,
      ...
      type,
      this,
      this.parent.generateNewId().toString(),
      { x: 0, y: 0 },
      "",
    );
  ...
  deleteNode(node: Node) {
    this.nodes.forEach((n) => {
      if (n.refinements) {
        n.refinements.refinements = n.refinements.refinements.filter(
          (r) => r.sourceId !== node.customId,
        );
      }
    });
...
export class GoalModel extends vscode.TreeItem {
  ...
  constructor(
    ...
    const info = actors.map((actor) => {
      ...
      return {
        name: name,
        missionNumber: parsedText[0],
        id: actor.id,
        nodes: actor.nodes,
        pos: { x: actor.x, y: actor.y },
        customProperties: actor.customProperties,
      };
    });
    this.missions = info.map(
      (info) =>
        new Mission(
          info.name,
          info.missionNumber,
  ...
  parseLinks() {
    return this.missions
      .map((mission) => mission.getLinks())
      .flat(Infinity) as gmTypes.Link[];
  }
