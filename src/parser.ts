...
type ParsedActor = Omit<Actor, "customProperties"> &
  Record<`MR_${string}`, string>;
...
export function convertJSON2JS(input: string): GoalModel {
  ...
  const size = [
    parseInt(gm.attributes.pageWidth),
    parseInt(gm.attributes.pageHeight),
  ];
  ...
  nodes.forEach((node: any) => {
    ...
    if (["istar.Goal", "istar.Task"].includes(type)) {
      ...
      parsedGm.actors
        .filter((actor: any) => actor.id === gmNode.parent)[0]
        .nodes.push(gmNode);
